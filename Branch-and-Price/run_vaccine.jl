...
for values in TD
    ...
    tol = 1e-3
    global batch_bound = values[3]
    include("vaccines.jl")
    include("main.jl")
    t_val = @timed begin
        println("Going for T = ", T, " D = ", num_treatment_vals, " B = ", batch_bound)
    ...
    if cg_ip
        ...
        #Costs for Bi-Partite Branchings
        push!(output, nodes_x)
        push!(output, LB_leaves_x)
        push!(output, UBx)
        push!(output, real_ub_bp_x)

        #Costs for Tri-Partite Branchings
        push!(output, nodes_z)
    ...
    output_master_df = DataFrame([getindex.(output_master, i) for i in 1:27], :auto, copycols=false)
    rename!(output_master_df, ["batched_norm","T","D","N","states","t_state","t_mp_root","t_dp_root","t_cgip", "t_mp_x", "t_dp_x", "t_bp_x", "t_mp_z", "t_dp_z", "t_bp_z","t_total","lb_cg", "ub_cgip", "real_cg", "nodes_x", "LB_x", "UB_x", "real_bp_x", "nodes_z", "LB_z", "UB_z", "real_bp_z"])
    CSV.write("results_vaccines/vaccines_"*string(T)*"_"*string(X)*"_"*string(bound)*".csv", output_master_df)
end
