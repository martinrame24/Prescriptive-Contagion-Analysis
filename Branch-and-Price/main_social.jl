...
function check_fractional_sol_xy(model, z_pi, decision_var, worst)
    fra_region = -1
    fra_time = -1
    fra_lbval = -1
...
function branch_and_price(initial_state, state_space, next_states, state_costs, ip; ode_accuracy = :auto, ode_accuracy_level = (1e-3, 1e-3), upper_bound = Inf, optimality_gap = 0)
    BB_nodes = [0]
    LB_leaves = []
    ...
    while (true) & (elapsed_time < 3600*2)
        popfirst!(BB_nodes)
        ...
        fra_region_y, fra_time_y, fra_lbval_y, fra_decision_y, assign_it = check_fractional_sol_xy(model, z, 2, worst)
        fra_region_x = fra_region_y
        #println("BP T = ", T, " D = ", num_treatment_vals)
        ...
        if (((LB-UB)/LB) > optimality_gap) & (fra_region_y > 0) & (feasible)
            if fra_region_y > 0 #Solution is still fractional for vaccines
                #We add left node first
                global TOT_NODE += 1
                LB_NODE[TOT_NODE] = LB
                PRE_NODE[TOT_NODE] = CUR_NODE
                SIGN_NODE[TOT_NODE] = "left"
                THRE_NODE[TOT_NODE][1] = fra_region_y
                THRE_NODE[TOT_NODE][2] = fra_time_y
                THRE_NODE[TOT_NODE][3] = fra_lbval_y
                THRE_NODE[TOT_NODE][4] = fra_decision_y
                ...
                THRE_NODE[TOT_NODE][1] = fra_region_y
                THRE_NODE[TOT_NODE][2] = fra_time_y
                THRE_NODE[TOT_NODE][3] = fra_lbval_y + 1
                THRE_NODE[TOT_NODE][4] = fra_decision_y
        ...
        if (((LB-UB)/LB) > optimality_gap) & (fra_region_y < 0) & (feasible)
            fra_region_x, fra_time_x, fra_lbval_x, fra_decision_x, assign_it = check_fractional_sol_xy(model, z, 1, worst)
            if fra_region_x > 0 #Solution is still fractional for budget
                #We add left node first
                global TOT_NODE += 1
                LB_NODE[TOT_NODE] = LB
                PRE_NODE[TOT_NODE] = CUR_NODE
        ...
        if (((LB-UB)/LB) > optimality_gap) & (fra_region_x < 0) #An integer solution is found
...
function branch_and_price_z(initial_state, state_space, next_states, state_costs, ip; ode_accuracy = :auto, ode_accuracy_level = (1e-3, 1e-3), upper_bound = Inf, optimality_gap = 0)
    BB_nodes = [0]
    LB_leaves = []
    best_assign_it = [[repeat([0], N_DECISION) for t in 1:T] for i in 1:N_REGION]
    global C_BUDGET = []
    ...
    while (true) & (elapsed_time < 3600*2)
        ...
        global C_BUDGET = []
        global C_REGION = []
        #println("Branchin range region 9: ", BRANCHING_RANGE[9])
        (model, z, C_BUDGET, C_REGION, obj, obj_ip, IP, z_final, allocations, t_dp, t_mp), t_cg = @timed column_generation_with_ip(initial_state, state_space, next_states, state_costs, C_BUDGET, C_REGION, ode_accuracy = ode_accuracy, ode_accuracy_level = ode_accuracy_level)
        ...
        if ((LB-UB)/LB) <= optimality_gap
            ...
            #println("LB: ", LB)
            #println("UB: ", UB)
            #println("Gap: ", (LB-UB)/LB)
            println("This node was pruned by bound")
        end
        
        if (((LB-UB)/LB) > optimality_gap) & (fra_region_y > 0) & (feasible)
            if fra_region_y > 0 #Solution is still fractional for vaccines
                println("Branching on y at time ", fra_time_y, " for product ", fra_region_y)
                #We add left node first
                global TOT_NODE += 1
                LB_NODE[TOT_NODE] = LB
                PRE_NODE[TOT_NODE] = CUR_NODE
        ...
        if (((LB-UB)/LB) > optimality_gap) & (fra_region_y < 0) & (feasible)
            println("Integer for y, branching on x")
            fra_region_x, fra_time_x, fra_lbval_x, fra_decision_x, assign_it = check_fractional_sol_xy(model, z, 1, worst)
            if fra_region_x > 0
                println("Branching on x at time ", fra_time_x, " for product ", fra_region_x)
...
function branch_and_price_small(initial_state, state_space, next_states, state_costs, ip; ode_accuracy = :auto, ode_accuracy_level = (1e-3, 1e-3), upper_bound = Inf, optimality_gap = 0)
    BB_nodes = [0]
    LB_leaves = []
    best_assign_it = [[repeat([0], N_DECISION) for t in 1:T] for i in 1:N_REGION]
    global C_BUDGET = []
...
function branch_and_price_z_small(initial_state, state_space, next_states, state_costs, ip; ode_accuracy = :auto, ode_accuracy_level = (1e-3, 1e-3), upper_bound = Inf, optimality_gap = 0)
    BB_nodes = [0]
    LB_leaves = []
    best_assign_it = [[repeat([0], N_DECISION) for t in 1:T] for i in 1:N_REGION]
    global C_BUDGET = []
